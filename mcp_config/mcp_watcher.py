...
from mcp_config.mcp_sender import MCPConfigSender  # 서버 전송 활성화

logger = logging.getLogger(__name__)


class MCPConfigChangeHandler(FileSystemEventHandler):
    """MCP 설정 파일 변경 이벤트 핸들러 - 서버 전송 모드"""

    def __init__(self, sender: MCPConfigSender = None, debounce_seconds: float = 2.0):
        """
        Args:
            sender: MCP 설정 전송 클래스 (None이면 디버깅 모드)
    ...
    def on_modified(self, event):
        ...
        try:
            ...
            # 서버로 전송
            if self.sender:
                logger.info(f"서버로 {service.upper()} 설정 전송 중...")
                success = self.sender.send_config(service, file_path, raw_content, status="activate")
                if success:
                    logger.info(f"✓ {service.upper()} 서버 전송 성공")
                else:
                    logger.error(f"✗ {service.upper()} 서버 전송 실패")
    ...
    def on_deleted(self, event):
        ...
        logger.info(f"{service.upper()} 삭제 감지")

        # 서버로 삭제 알림 전송
        if self.sender:
            logger.info(f"서버로 {service.upper()} 삭제 알림 전송 중...")
            success = self.sender.send_config(service, file_path, "[FILE DELETED]", status="delete")
            if success:
                logger.info(f"✓ {service.upper()} 삭제 알림 전송 성공")
            else:
                logger.error(f"✗ {service.upper()} 삭제 알림 전송 실패")
...
class MCPConfigWatcher:
    """MCP 설정 파일 감시자 - 서버 전송 모드"""

    def __init__(self, server_url: str = None, verify_tls: bool = False):
        """
        초기화
        Args:
            server_url: 서버 URL (None이면 디버깅 모드)
            verify_tls: TLS 인증서 검증 여부
        """
        # 서버 전송 클래스 초기화
        self.sender = MCPConfigSender(server_url, verify_tls) if server_url else None
        self.event_handler = MCPConfigChangeHandler(sender=self.sender)
    ...
    def _send_initial_configs(self):
        """초기 MCP 설정 출력 및 서버 전송"""
        logger.info("=== 초기 MCP 설정 출력 및 서버 전송 ===")
        ...
        for service, config_path in config_files:
            try:
                ...
                logger.info(f"{service.upper()} 초기 설정 읽기 완료 ({len(raw_content)} bytes)")

                # 서버로 전송
                if self.sender:
                    logger.info(f"서버로 {service.upper()} 초기 설정 전송 중...")
                    success = self.sender.send_config(service, config_path, raw_content, status="activate")
                    if success:
                        logger.info(f"✓ {service.upper()} 초기 설정 전송 성공")
                    else:
                        logger.error(f"✗ {service.upper()} 초기 설정 전송 실패")
